...
class Vorace_agent:
    ...
    def __init__(self, typeP, batch_size=0, callbacks_list = None, n_classifiers=10):
        
        if typeP==4:
            typeP=random.randint(0,3)
        ...
        if typeP==0:
            ...
            self.classifier =  BinaryRelevance(
                                classifier= choice([SVC(kernel=choice(['rbf', 'linear']), C=c_value, gamma='auto'), DecisionTreeClassifier(criterion = choice(['gini','entropy']),max_depth=random.randint(5,25), random_state=None)]),
            ...
            self.classifier = ClassifierChain(
                                classifier = choice([SVC(kernel=choice(['rbf', 'linear']), C=c_value, gamma='auto'), LogisticRegression(random_state=None)]),
                                require_dense = [False, True]
                                )
            self.initialState = clone(self.classifier)                               
            
        elif typeP==2:
            self.classifier = MLkNN(
                                k = random.randint(1,4),
                                s = random.uniform(0.5, 1.50)
                                )
            self.initialState = clone(self.classifier)  

        elif typeP==3:
            self.classifier = RakelD(
                                base_classifier=choice([GaussianNB(), RandomForestClassifier(max_depth=random.randint(2,4), criterion = choice(['gini','entropy']), random_state=None)]),
                                base_classifier_require_dense=[True, True],
                                labelset_size=random.randint(2,4)
                            )
            self.initialState = clone(self.classifier)
...
class Vorace:
    ...
    def __init__(self, n_models, profile_type, batch_size, callbacks_list=None):
        ...
        for i in range(n_models):
            self.models.append(Vorace_agent(profile_type, batch_size, callbacks_list))
    ...
    def fit(self,x,y, y_oneHot=None, bagging=False):
        ...
        if bagging:
            ...
            for i in range(len(self.models)):
                ...
                tmp_hist =  self.models[i].score(x, y)
                self.weights.append(tmp_hist)
    ...
    def predict(self, x, bestClassifier=False, voting="Sum", argMax=False, weighted=False, tiebreak="best", epsilon=False):
        ...
        if bestClassifier:
            ...
            if weighted==False:
                y_pred, temp_fscores=self.votingSystem(voting,self.models,x,  proba=True)
            else:
                y_pred, temp_fscores=self.votingSystem(voting,self.models,x, proba=True, weights=self.weights)
    ...
    def votingSystem(self,scoring, listModels, x, y=[], classes=10, best_index=0, weights=None, proba=False):
        ...
        for i in range(len(self.models)):
            ...
            #preferences = np.sum(preferences, axis=0)
        ...
        elif scoring is "Approval":
            #print("Preferences")

            classes = preferences[0].shape[1]
            
            #preferences[preferences >= 0.5] = 1
            #preferences[preferences < 0.5] = 0
            
            #scores=np.sum(preferences, axis=2)
